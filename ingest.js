...
const QDRANT_COLLECTION_NAME = 'cc_t_and_c'; // Your chosen collection name
...
async function getEmbeddings(texts) {
    console.log(`[Embeddings] Generating for ${texts.length} chunks using ${EMBEDDING_MODEL}...`);
    ...
    try {
        ...
        for (const chunk of allParsedChunks) {
            ...
            const metadata = {
                ...
                card_name: sourceMetadata.card_name,
                ...
                section_header: chunk.sectionHeader || 'Unknown Section',
            };
        ...
        for (let i = 0; i < embeddings.length; i++) {
            pointsToUpsert.push({
                ...
                vector: embeddings[i],
                payload: chunkMetadataList[i], // Store all metadata in payload
            });
        }
